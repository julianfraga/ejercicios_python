...
#%%
# Ejercicio 6.10 (16/9/2022)

# Agrego laposibilidad de sólo calcular alturas. 
def alt_diam(arboleda, especie, solo_altura=False):
    if solo_altura:
        lista=[(float(arbol['altura_tot'])) for arbol in arboleda if arbol['nombre_com']==especie]
    else:
...
altos = alt_diam(arboleda, 'Jacarandá', solo_altura=True)
plt.clf()
...
# =============================================================================
# Ejercicio 6.11
# Para el scatter plot voy a transformar la lista de tuplas de alt_diam
# en una matriz de nx2 en la cual la primera columna son las alturas en 
# metros y la segunda los diámetros en cm
# =============================================================================
import numpy as np
def scatter_hd(arboleda, especie):

    altdiam=alt_diam(arboleda, especie)
    matriz=np.reshape(np.array(altdiam),newshape=(len(altdiam), 2))
    h, d=matriz[:,0], matriz[:,1]
    plt.clf()
    plt.grid(color='lightgray', linestyle='dashdot')
    plt.scatter(h, d, marker='.')
    plt.xlabel('Altura [m]')
    plt.ylabel('Diámetro [cm]')
    plt.title(f'Scatter de alturas y diámetros de la especie {especie}')
    return
scatter_hd(arboleda, 'Jacarandá')